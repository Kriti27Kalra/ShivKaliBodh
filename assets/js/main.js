document.addEventListener("DOMContentLoaded", (event) => {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const nav = document.querySelector(".ul-header-nav");

    if (nav) {
      const items = nav.querySelectorAll(".has-sub-menu");

      items.forEach((item) => {
        // Remove submenu content
        const submenu = item.querySelector(".ul-header-submenu");
        if (submenu) submenu.remove();

        // Keep only the top-level link
        const link = item.querySelector("a");
        if (link) {
          // Replace whole .has-sub-menu div with just the link
          const wrapper = document.createElement("div");
          wrapper.appendChild(link.cloneNode(true));
          item.replaceWith(wrapper);
        } else {
          item.remove(); // No <a>? Just remove the whole block
        }
      });
    }
  });
    ...
    //     },
    // });
    // { © 2025 Krishna Raj | GitHub: https://github.com/krishna-raz }
...
//{ © 2025 Krishna Raj | GitHub: https://github.com/krishna-raz }
  ...
  document.querySelector('.ul-sidebar-closer')?.addEventListener('click', () => {
    document.querySelector('.ul-sidebar')?.classList.remove('active');
  });
